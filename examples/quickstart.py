...
pj = ros.Project.new_current(name = 'quickstart')
...
an = ros.Analysis.current_analysis(pj_id = pj.id) # For the current project.
lg = ros.Logsheet.current_logsheet(an_id = an.id) # For the current analysis.
ds = ros.Dataset.current_dataset(pj_id = pj.id) # For the current project.
...
lg.col_header[ros.Subject] = 'Subject Codename' # Key is the class itself, value is the column header.
lg.col_header[ros.Visit] = 'Visit Number'
...
vr = ros.Variable(id = "VR000000_001")
...
# d. Add input and output arguments to the process.
pr.set_inputs(vr1)
vr2 = ros.Variable(id = "VR000000_002")
pr.set_outputs(vr2)
